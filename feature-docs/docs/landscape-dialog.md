...
Диалог с read-only информацией о штрихкоде. Открывается из `MainBottomSheet` по нажатию на иконку горизонтального просмотра. Боттомшит при этом остаётся открытым — диалог показывается поверх него.
...
- `Surface(shape = lRoundedCornerShape)` — ширина 95% экрана
- Левая секция (`width = 110.dp`): изображение штрихкода, генерируется через `BarGenerator` (Koin inject). До готовности — `CircularProgressIndicator`
- Правая секция (`weight(1f)`): тип баркода (labelMedium), поля InfoRow, теги

**Правила отображения полей:**
| Поле | Отображается |
|------|-------------|
| Тип (`barcodeType`) | всегда |
| Content | только если `isNotEmpty()` |
| Name | только если `isNotEmpty()` |
| Description | только если `isNotEmpty()` |
| Tags | только если список непустой |

**Теги:** `BargenChip(selectable = false)` — кликабельность и haptic-фидбек отключены.

**Кнопка закрытия:** `IconButton` с иконкой `outline_cancel_24`, расположен в правом верхнем углу. Закрывает диалог, боттомшит остаётся открытым.
...
- Правая колонка не скроллируется — для типичных данных (короткий контент, название, пара тегов) этого достаточно. При необходимости добавить `verticalScroll` на `BarcodeInfoSection`.
