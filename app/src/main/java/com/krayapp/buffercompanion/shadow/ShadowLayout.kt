package com.krayapp.buffercompanion.shadow
	...
	init {
		if (!isInEditMode) {
			val attributes = context.obtainStyledAttributes(
				...
				shadow_color = attributes.getColor(
					R.styleable.ShadowLayout_shadow_color, ContextCompat.getColor(context,
						R.color.shadow_color
					))
				shadow_with_color =
					attributes.getBoolean(R.styleable.ShadowLayout_shadow_with_color, false)
				shadow_with_content =
					attributes.getBoolean(R.styleable.ShadowLayout_shadow_with_content, true)
				shadow_with_dpi_scale =
					attributes.getBoolean(R.styleable.ShadowLayout_shadow_with_dpi_scale, true)
				shadow_with_css_scale =
					attributes.getBoolean(R.styleable.ShadowLayout_shadow_with_css_scale, true)
				shadow_x_shift =
					attributes.getDimension(R.styleable.ShadowLayout_shadow_x_shift, 0f)
				shadow_y_shift =
					attributes.getDimension(R.styleable.ShadowLayout_shadow_y_shift, 0f)
				shadow_downscale =
					attributes.getFloat(R.styleable.ShadowLayout_shadow_downscale, 1f)
				shadow_radius = attributes.getFloat(R.styleable.ShadowLayout_shadow_radius, 6f)


			background = attributes.getDrawable(R.styleable.ShadowLayout_shadow_background)
				?: ContextCompat.getDrawable(getContext(), R.drawable.background_round_white)
			attributes.recycle()
		}
