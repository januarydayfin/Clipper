...
class StateManagerImpl(private val remover: EffectRemover): StateManager {
    ...
    private suspend fun showSettingsBottomsheet() {
        withIO {
            val currentState = _state.value
            val newState =
                currentState.copy(showSettingsBottomSheet = ShowSettingsBottomsheet(show = true))
            _state.emit(newState)
        }
    }
    ...
    override suspend fun recycleEffect(effect: Effect) {
        withIO {
